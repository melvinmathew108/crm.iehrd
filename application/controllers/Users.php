...
class Users extends My_Controller {
	...
	public function index() {
        ...
        $this->data['groups'] = $this->common->get_all('groups');
	...
	public function create() {
        ...
        $this->data['array_branch'] = $this->common->get_all('branch');
...
//                $zone_ids = $this->input->post('zone_ids');
...
//                $zone_ids = implode(',', $zone_ids);
    ...
    function targets( $user_salt_id = 0) {
        ...
        $this->data['row_user'] = $row;

        $this->data['arr_targets'] = $arr_targets = $this->common->get_all('target', ['sales_id'=>$id]);

        $this->data["result"] = $this->common->get_all('target', ['sales_id'=>$id],
            'SQL_CALC_FOUND_ROWS *', '', 30, $this->uri->segment(5));

        $total = $this->common->get_search_count();

        $config['base_url'] = site_url('users/targets/'.encrypt($id).'/page/');
        ...
        $this->load->view('templates/header',$this->data);
        $this->load->view('templates/sidebar',$this->data);
        $this->load->view('users/list_target',$this->data);
        $this->load->view('templates/footer',$this->data);

    }

    public function create_target($user_id){
        ...
        $this->data['page_title'] = 'New Target';
        $this->data['menu'] = 'users';
        $this->data['submenu'] = 'add';
        $this->data['action'] = 'add';
        ...
        if($_POST) {
            ...
            if( $this->form_validation->run() == false ){
                ...
                $id = $this->common->insert( 'target', $data_arr );

                $this->session->set_flashdata('msg','Saved!');

                redirect('users/targets/'.encrypt($user_id));
        ...
        $this->load->view('templates/header',$this->data);
        $this->load->view('templates/sidebar',$this->data);
        $this->load->view('users/form_target',$this->data);
        $this->load->view('templates/footer',$this->data);


    }

    public function delete_target( $user_salt_id = 0 ) {
        ...
        $row = $this->common->get('target', array( 'id' => $id ), 'array');

        $this->common->delete( 'target', array( 'id' =>  $id ) );

        $this->session->set_flashdata('msg','Target deleted successfully');
